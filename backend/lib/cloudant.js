 ...
 * @param {String} adiinfo - the adiinfo of the item
 * @param {String} nop - the No. of People In Shop available 
 ...
 */
function create(type, name, adiinfo, nop, location, contact, userID) {
    return new Promise((resolve, reject) => {
        let itemId = uuidv4();
 ...
 * @param {String} name - the name of the item
 * @param {String} adiinfo - the adiinfo of the item
 * @param {String} nop - No. Of People In Shop  
 * @param {String} location - the GPS location of the item
 * @param {String} contact - the contact info 
 * @param {String} userID - the ID of the user 
 * 
 ...
 * when the call to the DB completes
 */
function update(id, type, name, adiinfo, nop, location, contact, userID) {
    return new Promise((resolve, reject) => {
        db.get(id, (err, document) => {
            if (err) {
                ...
                if (adiinfo) {item["adiinfo"] = adiinfo} else {item["adiinfo"] = document.adiinfo};
                if (nop) {item["nop"] = nop} else {item["nop"] = document.nop};
